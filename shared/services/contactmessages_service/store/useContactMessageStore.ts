...
import { ContactMessageFormDataType } from '../type/contact_message_formdata_type'
...
type ContactMessageState = {
  ...
  submitContactMessage: (data: ContactMessageFormDataType) => Promise<boolean>
...
export const useContactMessageStore = create<ContactMessageState>((set) => ({
  ...
  submitContactMessage: async (data) => {
    ...
    try {
      const response = await contactMessagesService.post({
        name: data.name,
        email: data.email,
        phone: data.phone,
